const state = {
  ...
  selectedCategories: new Set(['Genel Kültür', 'Genel Yetenek', 'Eğitim Bilimleri']),
};
...
const el = {
  ...
  categorySelectionHint: document.getElementById('categorySelectionHint'),
...
async function startSolo() {
  if (!state.selectedCategories.size) {
    el.categorySelectionHint?.classList.remove('hidden');
    return;
  }
  el.categorySelectionHint?.classList.add('hidden');

  const selected = [...state.selectedCategories];
  const chunks = await Promise.all(selected.map((main) => {
    const query = new URLSearchParams({
      main,
      sub: 'all',
      difficulty: el.difficultySelect.value,
    }).toString();
    return api(`/api/questions?${query}`);
  }));

  const merged = chunks.flatMap((chunk) => chunk.questions || []);
  const unique = [];
  const seen = new Set();
  merged.forEach((q) => {
    const key = `${q.stem}|${q.mainCategory}|${q.subCategory}`;
    if (!seen.has(key)) {
      seen.add(key);
      unique.push(q);
    }
  ...
  if (!shuffled.length) {
    alert('Seçilen kategori ve zorlukta soru bulunamadı.');
    return;
  }
...
function setupCategorySelection() {
  el.categoryCards.forEach((card) => {
    card.addEventListener('click', () => {
      ...
      el.categorySelectionHint?.classList.toggle('hidden', state.selectedCategories.size > 0);
      el.categoryCards.forEach((item) => {
        const isActive = state.selectedCategories.has(item.dataset.category);
